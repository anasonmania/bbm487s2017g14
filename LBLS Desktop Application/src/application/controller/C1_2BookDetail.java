...
public class C1_2BookDetail implements Initializable {
	...
	public void borrowBook(ActionEvent event) throws SQLException, IOException{
		...
		if(borrowCount<3){
			...
			BorderPane borrowPane = (BorderPane) FXMLLoader.load(this.getClass().getResource("../view/DialogFailed.fxml"));
	...
	public void reserveBook(ActionEvent event) throws SQLException, IOException{
		PreparedStatement countQuery = Main.con.prepareStatement("SELECT COUNT(book) "
				+ "AS reservationcount FROM reservation WHERE book = " + Main.selectedBook.getid());
		ResultSet countResult = countQuery.executeQuery();
		int order = 1;
		while(countResult.next()){
			order = countResult.getInt("reservationcount");
		}

		PreparedStatement reservationQuery = Main.con.prepareStatement("INSERT INTO "
				+ "reservation (`book`, `user`, `order`) VALUES (?, ?, ?)");
		reservationQuery.setInt(1, Main.selectedBook.getid());
		reservationQuery.setInt(2, Main.currentUser.getUserId());
		reservationQuery.setInt(3, order++);
