...
public class C1_3BorrowDialog implements Initializable {
	...
	public void borrow(ActionEvent event) throws SQLException{
		...
		PreparedStatement borrowQuery = Main.con.prepareStatement("INSERT INTO "
				...
				borrowQuery.setDate(3, DBFormatController.dateToDatabase((LocalDate)dpBorrowdate.getValue()));
				borrowQuery.setDate(4, DBFormatController.dateToDatabase((LocalDate)dpReturndate.getValue()));

				if(dpReturndate.getValue()!=null)
					borrowQuery.execute();

			PreparedStatement	updateBookQuery = Main.con.prepareStatement(
						"UPDATE book SET owner = ?, isavailable = ? WHERE id = ? ");
			updateBookQuery.setInt(1, Main.currentUser.getUserId());
			updateBookQuery.setBoolean(2, false);
			updateBookQuery.setInt(3, Main.selectedBook.getid());
